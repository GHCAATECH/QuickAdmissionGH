...
function repExcel(title,headers,rows,totalStudents){
  ...
  var schoolName=exportSchoolName(), admissionYear=exportAdmissionYear(), footerHead=exportHeadLabel();
  var total=(typeof totalStudents==='number'&&!isNaN(totalStudents))?totalStudents:rows.length;
  var html='<table border="1" style="border-collapse:collapse;width:100%"><thead><tr>'+headers.map(h=>`<th style="border:1px solid #1f2937;padding:7px 9px;background:linear-gradient(135deg,#14532d 0%,#16a34a 100%);color:#ffffff;text-align:left">${h}</th>`).join('')+'</tr></thead><tbody>'+rows.map(r=>'<tr>'+r.map(c=>`<td style="border:1px solid #1f2937;padding:7px 9px">${c==null?'':c}</td>`).join('')+'</tr>').join('')+'</tbody></table>';
...
function repPrint(title,headers,rows,totalStudents){
  ...
  var schoolName=exportSchoolName(), admissionYear=exportAdmissionYear(), footerHead=exportHeadLabel();
  var total=(typeof totalStudents==='number'&&!isNaN(totalStudents))?totalStudents:rows.length;
  w.document.write('<html><head><title>'+title+'</title><style>body{font-family:Arial,sans-serif;padding:24px}.pdf-head{margin-bottom:14px;padding:14px 16px;border-radius:14px;background:linear-gradient(135deg,#14532d 0%,#16a34a 100%);color:#fff}.pdf-head h2{margin:0 0 4px}.pdf-sub{font-size:12px;color:rgba(255,255,255,.9)}.pdf-foot{margin-top:14px;font-size:12px;color:#374151;display:flex;justify-content:space-between;gap:18px}table{width:100%;border-collapse:collapse;font-size:12px}th,td{border:1px solid #ccc;padding:6px 8px;text-align:left}th{background:linear-gradient(135deg,#14532d 0%,#16a34a 100%);color:#fff}</style></head><body><div class="pdf-head"><h2>'+safeHtml(schoolName)+' - '+safeHtml(title)+'</h2><div class="pdf-sub">'+(admissionYear?('Admission Year: '+safeHtml(admissionYear)):'Admission Year: -')+'</div></div><table><thead><tr>'+headers.map(h=>'<th>'+h+'</th>').join('')+'</tr></thead><tbody>'+rows.map(r=>'<tr>'+r.map(c=>'<td>'+(c==null?'':c)+'</td>').join('')+'</tr>').join('')+'</tbody></table><div class="pdf-foot"><div>'+safeHtml(footerHead)+'</div><div>Total Students: '+total+'</div></div></body></html>');
  w.document.close(); setTimeout(function(){w.print();},300);
}
