...
begin
  ...
  if final_admission_no is null then
    perform pg_advisory_xact_lock(
      hashtext(
        sid::text || '|' || coalesce(stu.programme_id::text, '') || '|' || programme_code || '|' || admission_year_text
      )
    );
    ...
    where st.school_id = sid
      ...
      and (
        upper(coalesce(st.admission_no, '')) like programme_code || '/' || admission_year_text || '/%'
        or (
          st.programme_id is not distinct from stu.programme_id
    ...
    final_admission_no := programme_code || '/' || admission_year_text || '/' || lpad((prior_count + 1)::text, 3, '0');
  end if;
