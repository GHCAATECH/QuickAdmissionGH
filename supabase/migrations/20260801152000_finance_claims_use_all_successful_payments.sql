-- Finance is earned when a student's token payment succeeds. Completing the
-- personal-record form is a separate admission step and must not hide revenue.
...
revoke all on function public.apply_finance_claim(uuid, integer) from public, anon;
