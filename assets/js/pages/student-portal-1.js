...
function syncPortalChrome(){
  ...
  if(!topbar||!document.getElementById('s-app').classList.contains('active'))return;
...
function showScreen(id){
  if(tenantPortalRequested()&&TENANT_PORTAL.checked&&!tenantSchoolId()&&id!=='s-login'){
    toast('This school portal is not available.','warn');
    id='s-login';
  }
  document.querySelectorAll('.screen').forEach(s=>s.classList.toggle('active',s.id===id));
  document.body.classList.toggle('student-dashboard-active',id==='s-app');
  window.scrollTo(0,0);
  if(id==='s-app') requestAnimationFrame(syncPortalChrome);
  if(id==='s-purchase'){try{showPendingBanner();}catch(e){}}
}
...
function logout(){stopWatchDeletion();session=null;STU=null;activeStudentSessionKey='';clearStudentSession();writeSubmissionSuccess(null); showScreen('s-login');$('login-token').value='AS7K9X2Q';$('login-index').value='100000000026';}
...
function buildSchoolPickerOptions(key,filterText){
  ...
  host.innerHTML=list.map(function(s){
    ...
    return `<button class="login-school-option${isActive?' active':''}" type="button" data-qa-onclick="pickSchoolPickerValue('${escapeAttr(String(key))}','${escapeAttr(String(s.id))}')"><span><strong>${name}</strong>${code&&code!=='&mdash;'?`<small>${code}</small>`:''}</span></button>`;
  }).join('');
}
...
async function loadSchools(){
  ...
  const ids=['login-school','p-school-sel','r-school'];
...
async function initStudentPortal(){
  try{
    await Promise.allSettled([loadPaystackConfig(),loadSchools(),bootStudent()]);
  }finally{
    finishStudentBoot();
  }
}
